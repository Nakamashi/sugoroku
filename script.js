...
const boardSpaces = [
  ...
  { id: 8, phrase: activityPhrases[8], x: 90, y: 64, next: [9, 16] },
  ...
  { id: 12, phrase: activityPhrases[12], x: 28, y: 88, next: [13] },
  { id: 13, phrase: activityPhrases[13], x: 12, y: 90, next: [0] },
  { id: 14, phrase: activityPhrases[14], x: 34, y: 66, next: [10] },
  { id: 15, phrase: activityPhrases[15], x: 52, y: 66, next: [14] },
  { id: 16, phrase: activityPhrases[16], x: 70, y: 66, next: [15] },
...
function validateBoardGraph() {
  const stack = [];
  const visitedEdges = new Set();

  function visit(spaceId) {
    const stackIndex = stack.indexOf(spaceId);
    if (stackIndex !== -1) {
      const cycle = stack.slice(stackIndex);
      if (!cycle.includes(0)) {
        console.warn(`Board graph has a loop that does not pass START: ${cycle.join(' -> ')} -> ${spaceId}`);
      }
      return;
    }
